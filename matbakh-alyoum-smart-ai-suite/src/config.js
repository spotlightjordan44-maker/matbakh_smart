...
export const CONFIG = {
  ...
  wa: {
    ...
    adminNumbers: uniqueAdminNumbers(
      parsePipeList(
        required("WA_ADMIN_NUMBERS", "962788328832,962799060405,962799080840"),
        ","
      ),
      [
    ...
    autoApproveAfterMinutes: Number(
      required("AUTO_APPROVE_DELIVERY_AFTER_MINUTES", "15")
    ),
  },
  ...
  orderFlow: {
    ...
    operatingStartHour: Number(required("OPERATING_START_HOUR", "14")),
    operatingEndHour: Number(required("OPERATING_END_HOUR", "20")),
    ...
    enableAbandonedReminder:
      required("ENABLE_ABANDONED_REMINDER", "false") === "true",
    abandonedReminderMinMinutes: Number(
      required("ABANDONED_REMINDER_MIN_MINUTES", "15")
    ),
    abandonedReminderMaxMinutes: Number(
      required("ABANDONED_REMINDER_MAX_MIN_MINUTES", "25")
    ),
    reminderLoopSeconds: Number(required("REMINDER_LOOP_SECONDS", "60")),
    followupLoopSeconds: Number(required("FOLLOWUP_LOOP_SECONDS", "90")),
    delayedCustomerMessageEnabled:
      required("DELAYED_CUSTOMER_MESSAGE_ENABLED", "true") === "true",
    delayedCustomerPhone: required(
      "DELAYED_CUSTOMER_PHONE",
      required("BUSINESS_PHONE", "962779960015")
    ),
  },
